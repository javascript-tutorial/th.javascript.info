...
แต่จริงๆ แล้ว หลายครั้งเราอยากได้ `undefined` กลับมาแทน error (หมายถึง "ไม่มีถนน")
...
นี่แหละคือเหตุผลว่าทำไม optional chaining `?.` ถึงถูกเพิ่มเข้ามาในภาษา — เพื่อแก้ปัญหานี้ให้จบในคราวเดียว!
...
เพราะงั้น ถ้ามีฟังก์ชันหรือคำสั่งอื่นต่อทางขวาของ `?.` ก็จะไม่ทำงานเลย
...
แต่ต้องใช้ `?.` อย่างระวังนะ — ใช้เฉพาะตรงที่ตามตรรกะของโค้ดแล้วยอมรับได้ว่าส่วนซ้ายอาจไม่มีอยู่ ไม่งั้น error ที่ควรเกิดจะถูกซ่อนเงียบ พอไปตามหาสาเหตุทีหลังจะปวดหัว

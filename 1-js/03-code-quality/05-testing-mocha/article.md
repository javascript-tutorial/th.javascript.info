...
เรากำลังจะเพิ่มเทสต์อีกหลายอย่าง แต่ก่อนอื่นให้สังเกตว่าฟังก์ชันตัวช่วย `makeTest` และ `for` ควรถูกจัดกลุ่มไว้ด้วยกัน เราจะไม่ต้องใช้ `makeTest` ในเทสต์อื่นๆ มันจำเป็นแค่ใน `for` เท่านั้น: หน้าที่หลักร่วมกันของพวกมันคือตรวจสอบว่า `pow` ยกกำลังตามเลขชี้กำลังที่กำหนดได้อย่างไร
...
`describe` ที่ซ้อนกันจะกำหนด "กลุ่มย่อย" ใหม่ของเทสต์ ในผลลัพธ์เราจะเห็นการเยื้องที่มีหัวข้อกำกับ:

[iframe height=250 src="pow-4" edit border="1"]

ในอนาคตเราสามารถเพิ่ม `it` และ `describe` เพิ่มเติมในระดับบนสุดพร้อมฟังก์ชันช่วยเหลือของตัวเอง โดยที่พวกมันจะไม่เห็น `makeTest`

```smart header="`before/after` และ `beforeEach/afterEach`"
เราสามารถตั้งค่าฟังก์ชัน `before/after` ที่จะทำงานก่อน/หลังการรันเทสต์ และฟังก์ชัน `beforeEach/afterEach` ที่จะทำงานก่อน/หลัง `it` *ทุกครั้ง*
...
โดยทั่วไปแล้ว `beforeEach/afterEach` และ `before/after` มักใช้ในการเตรียมสภาพแวดล้อม, รีเซ็ตตัวนับ หรือทำอย่างอื่นระหว่างเทสต์ (หรือระหว่างกลุ่มเทสต์)
````

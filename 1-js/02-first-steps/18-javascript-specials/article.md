...
บทนี้รวบรวมสิ่งที่เราเรียนมาจนถึงตอนนี้ไว้แบบกระชับ พร้อมเน้นจุดที่ควรระวังเป็นพิเศษ
...
...แต่แม้ว่าเราจะใส่เครื่องหมายอัฒภาค "เพิ่มเติม" ไว้ที่ใดสักแห่ง ก็ไม่ถือเป็นข้อผิดพลาด — JavaScript จะข้ามมันไปเฉยๆ
...
typeof function(){} == "function" // ฟังก์ชันมีการจัดการพิเศษ
```
...
: ถาม `question` และแนะนำให้เลือกระหว่าง ตกลง และ ยกเลิก โดยคืนค่าการเลือกเป็น `true/false`
...
: AND ตรรกะ `&&` และ OR ตรรกะ `||` ประเมินแบบลัดวงจรและคืนค่าตรงที่หยุด (ไม่จำเป็นต้องเป็น `true/false`) ส่วน NOT ตรรกะ `!` แปลงตัวถูกกระทำเป็น boolean แล้วคืนค่าตรงข้าม

ตัวดำเนินการ Nullish coalescing  
: ตัวดำเนินการ `??` ใช้เลือกค่าที่กำหนดแล้วจากรายการตัวแปร ผลลัพธ์ของ `a ?? b` คือ `a` เว้นแต่ `a` จะเป็น `null/undefined` จึงได้ `b` แทน
...
: การตรวจสอบความเท่ากัน `==` สำหรับค่าที่มีชนิดต่างกันจะแปลงเป็นตัวเลข (ยกเว้น `null` และ `undefined` ซึ่งเท่ากันและไม่เท่ากับอะไรอื่น) ดังนั้นพวกนี้จึงเท่ากัน:
    ...
    ค่า `null` และ `undefined` พิเศษกว่าชนิดอื่น: เท่ากัน `==` กันเองและไม่เท่ากับอะไรอื่นเลย
...
โครงสร้าง "switch" ใช้แทนการตรวจสอบ `if` หลายครั้งได้ โดยใช้ `===` (เปรียบเทียบแบบเข้มงวด) ในการจับคู่ค่า
...
- ฟังก์ชันอาจมีตัวแปรภายใน (local variables): คือตัวแปรที่ประกาศไว้ใน body หรือในรายการพารามิเตอร์ ใช้ได้เฉพาะภายในฟังก์ชันนั้นเท่านั้น
...
นี่เป็นเพียงรายการโดยย่อ — ตอนนี้เราเรียนแค่พื้นฐานเท่านั้น ในบทต่อๆ ไปจะได้พบกับฟีเจอร์ที่พิเศษและลึกซึ้งยิ่งขึ้นอีกมากมาย
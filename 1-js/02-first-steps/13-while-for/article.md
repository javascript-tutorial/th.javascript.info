...
บ่อยครั้งที่เราต้องทำสิ่งเดิมซ้ำๆ
...
ตราบใดที่ `condition` เป็นจริง โค้ดใน loop body ก็จะทำงาน
...
การที่ loop body ทำงานหนึ่งรอบเรียกว่า *การวนซ้ำ (iteration)* ลูปในตัวอย่างข้างต้นวนซ้ำทั้งหมดสามรอบ

ถ้าไม่มีคำสั่ง `i++` ในตัวอย่างข้างต้น ลูปจะวนซ้ำไปเรื่อยๆ ในทางทฤษฎี แต่ในทางปฏิบัติ เบราว์เซอร์มีวิธีหยุดลูปแบบนี้ได้ และใน JavaScript ฝั่งเซิร์ฟเวอร์ก็สามารถยุติกระบวนการได้เช่นกัน

เงื่อนไขลูปเป็นนิพจน์หรือตัวแปรอะไรก็ได้ ไม่จำกัดแค่การเปรียบเทียบ — `while` จะประเมินเงื่อนไขและแปลงเป็นบูลีนเอง
...
ลูปจะทำงาน body ก่อน แล้วค่อยตรวจสอบเงื่อนไข หากเงื่อนไขเป็นจริงก็วนซ้ำต่อไปเรื่อยๆ
...
มาทำความเข้าใจแต่ละส่วนผ่านตัวอย่างกัน ลูปด้านล่างจะรัน `alert(i)` สำหรับ `i` ตั้งแต่ `0` จนถึง (แต่ไม่รวม) `3`:
...
กล่าวคือ `begin` ทำงานแค่ครั้งเดียว จากนั้นก็เข้าสู่ลูป: หลังตรวจสอบ `condition` แต่ละครั้ง `body` และ `step` จะทำงานตามลำดับ

ถ้าเพิ่งเริ่มเรียนเรื่องลูป ลองย้อนกลับไปดูตัวอย่างแล้วเขียนไล่ทีละขั้นตอนบนกระดาษ — ช่วยให้เข้าใจได้ดีขึ้นมากเลย
...
คำสั่ง `break` จะทำงานที่บรรทัด `(*)` ถ้าผู้ใช้ป้อนบรรทัดว่างหรือกด cancel — จะหยุดลูปทันทีและส่งการควบคุมไปที่บรรทัดแรกหลังลูป นั่นคือ `alert`
...
ใช้ได้เมื่อเราจัดการรอบปัจจุบันเสร็จแล้วและอยากข้ามไปรอบถัดไปเลย
...
เมื่อ `i` เป็นเลขคู่ `continue` จะหยุด body แล้วส่งการควบคุมไปยังรอบถัดไปของ `for` (พร้อมตัวเลขถัดไป) ทำให้ `alert` แสดงผลเฉพาะค่าเลขคี่เท่านั้น
...
ในแง่เทคนิค โค้ดนี้เหมือนกับตัวอย่างด้านบนทุกประการ ห่อโค้ดไว้ในบล็อก `if` แทนการใช้ `continue` ก็ได้

แต่ผลข้างเคียงคือ โค้ดจะซ้อนลึกขึ้นอีกหนึ่งชั้น (การเรียก `alert` อยู่ภายในวงเล็บปีกกา) ถ้าโค้ดภายใน `if` ยาวกว่าสองสามบรรทัด อาจทำให้อ่านโค้ดโดยรวมลำบากขึ้น
````

````warn header="ไม่มี `break/continue` ที่ด้านขวาของ '?'"
สังเกตว่า โครงสร้างทางไวยากรณ์ที่ไม่ใช่นิพจน์จะใช้กับ ternary operator `?` ไม่ได้ โดยเฉพาะ `break/continue` ไม่อนุญาตให้ใช้ในตำแหน่งนั้น
...
...โค้ดจะหยุดทำงาน: เกิด syntax error ขึ้น

นี่เป็นอีกเหตุผลที่ไม่ควรใช้ ternary operator `?` แทน `if`
````
...
จำเป็นต้องมีวิธีหยุดกระบวนการนี้เมื่อผู้ใช้ยกเลิกการป้อนข้อมูล

การใช้ `break` ธรรมดาหลัง `input` จะออกจากลูปชั้นในเท่านั้น — ยังไม่เพียงพอ ตรงนี้แหละที่ labels เข้ามาช่วย!
...
คำสั่ง `continue` ก็ใช้ร่วมกับ label ได้เช่นกัน ในกรณีนี้ การทำงานจะข้ามไปยังรอบถัดไปของลูปที่มี label กำกับ

````warn header="Labels ไม่อนุญาตให้ \"กระโดด\" ไปที่ใดก็ได้"
Labels ไม่ได้อนุญาตให้กระโดดไปยังตำแหน่งใดๆ ก็ได้ในโค้ด
...
...อย่างไรก็ตาม ในทางปฏิบัติ 99.9% ของเวลา `break` จะอยู่ภายในลูป เหมือนที่เห็นในตัวอย่างด้านบน
...
- `while` -- ตรวจสอบเงื่อนไขก่อนการวนลูปแต่ละรอบ
- `do..while` -- ตรวจสอบเงื่อนไขหลังการวนลูปแต่ละรอบ
- `for (;;)` -- ตรวจสอบเงื่อนไขก่อนการวนลูปแต่ละรอบ สามารถกำหนดค่าเริ่มต้นและปรับแต่งเพิ่มเติมได้

ในการสร้างลูป "ไม่มีที่สิ้นสุด" มักใช้โครงสร้าง `while(true)` ลูปแบบนี้ เช่นเดียวกับลูปอื่นๆ หยุดได้ด้วยคำสั่ง `break`

ถ้าไม่อยากทำอะไรในรอบปัจจุบันและต้องการข้ามไปรอบถัดไปเลย ใช้คำสั่ง `continue` ได้

`break/continue` รองรับการใช้ label ที่อยู่ข้างหน้าลูป — label คือวิธีเดียวที่ทำให้ `break/continue` ออกจากลูปซ้อนไปยังลูปชั้นนอกได้

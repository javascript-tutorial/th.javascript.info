...
2. **การประเมินผลแบบสั้น (Short-circuit evaluation)**

    ฟีเจอร์อีกอย่างของโอเปอเรเตอร์ OR `||` คือการประเมินที่เรียกว่า "แบบสั้น (Short-circuit)"
    ...
    ในตัวอย่างด้านล่างมีเพียง บรรทัดที่สองเท่านั้นที่ฟังก์ชัน `alert` ทำงาน
    ...
    *!*false*/!* || alert("printed");
    ...
    ในบรรทัดแรก OR `||` จะหยุดทำงานทันทีที่ค่าตัวแรกเป็น truthy ดังนั้นฟังก์ชัน `alert` จึงไม่ทำงาน

    และในบางครั้งเราต้องการให้ฟังก์ชันอะไรสักอย่างทำงาน เมื่อเงื่อนไขทางซ้ายเป็น falsy
...
จบแล้ว สรุปคือแปลงค่าเป็นบูลีน และส่งค่าตรงข้ามกลับไปนั่นคือ `true` จะเป็น `false` และ `false` จะเป็น `true` เรามาแนะนำอีกนิดนึงนั่นคือฟังก์ชันที่แปลงค่าชนิดใดๆให้เป็นบูลีน

ฟังก์ชัน `Boolean` เป็นฟังก์ชันที่มีมาให้พร้อมกับตัวภาษา ฟังก์ชันนี้จะเขียนเยอะขึ้นนิดนึง
